...
public class RegistrationCommand : IRequest<AuthResponse>
{
    ...
    public sealed class RegistrationCommandHandler : IRequestHandler<RegistrationCommand, AuthResponse>
    {
        ...
        public RegistrationCommandHandler(UserManager<User> userManager, RoleManager<Role> roleManager, IAuthService authService, IDateTimeProvider dateTimeProvider)
        {
        ...
        public async Task<AuthResponse> Handle(RegistrationCommand request, CancellationToken cancellationToken)
        {
            ...
            var newUser = new User(request.FirstName, request.LastName, request.Email, 
                request.BirthDate, _dateTimeProvider.UtcNow, (GenderType)request.Gender)
                {
            ...
            if (role == null)
            {
                throw new InternalServerException("Роли не существует");
            }
